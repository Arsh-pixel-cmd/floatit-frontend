...
export default function CanvasSidebar({ isSidebarOpen, setIsSidebarOpen, setCurrentPage, mode }) {
  const { animationState } = useWorkflowStore();
  ...
  return (
    <>
      ...
      <div className={`absolute top-4 bottom-4 w-[180px] bg-white rounded-xl border border-gray-200 p-3 flex flex-col z-30 transition-all duration-300 shadow-xl ${isSidebarOpen ? 'left-4' : '-left-[200px]'}`}>
        ...
        <p className="text-xs font-bold text-gray-900 mb-4 px-1">{flowTitle || 'Untitled Project'}</p>
        <nav className="space-y-0.5 mb-6">
          {[
            ...
            <button
              ...
              className="flex items-center gap-2.5 w-full px-2 py-1.5 rounded-md text-[13px] font-medium text-gray-500 hover:bg-gray-50 transition">
        ...
        {mode === 'template' && (
          <div className="px-1">
            <p className="text-xs font-bold text-gray-900 mb-3">Project Pipeline</p>
                ...
                return (
                  <div key={p} className="flex items-start gap-2.5">
                    <div className="flex flex-col items-center mt-0.5">
                      ...
                      {!isLast && <div className="w-px h-6 bg-gray-200 mt-1"></div>}
                    </div>
                    <div>
                      <p className="text-[12px] font-semibold text-gray-900 leading-tight capitalize">{p}</p>
